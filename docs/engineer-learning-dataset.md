...
После добавления четвёртой иерархии и пересчёта трёх фронтов v3 отдельная
LOPO-проверка дала средний regret 0,015080 и top-1/top-3 = 2/3. Эти значения
относятся к **новым фронтам**, поэтому их нельзя приписывать прежней модели на
старом наборе точек. Все три слабые метки всё ещё относятся к К09. Доступная
проверенная ridge-модель может автоматически задать **начально показанный**
вариант полного фронта в web; конструктор может переключить его. Это не
автоматическое принятие инженерного решения: `placement_eligible=false`,
блокирующие проверки и необходимость инженерного утверждения сохраняются.

### Аудит КЖ00.С7-8

Просмотрены все три страницы `1-КЖ00.С7-8/л9-11.pdf`: это спецификация
фундаментной плиты с прямыми и гнутыми Г-, П- и Д-образными позициями,
приямками и наклонными стержнями. Полный итог этого PDF несопоставим с
текущей straight-only регрессией по дополнительным прямым стержням; OCR чисел
не проводился, новую независимую слабую метку не заявляем. После исправления
чтения серого ACI 9 строгий preflight четырёх DXF и парных PNG проходит 4/4
на общем mesh из 4429 КЭ при сохранённых SHA256 источников:
`artifacts/zone_merge_legacy_s78/preflight-gray-fixed.json`. Исходный
`preflight.json` с результатом 2/4 сохранён для сравнения. Затем полный
zone-merge сценарий с теми же DXF/PNG, SHA256 и исследовательскими фазами
создал `artifacts/zone_merge_legacy_s78/full-plate-zone-merge.json`: 141 точка,
все прошли `validated_front` и проверку покрытия. Минимум массы — 392 зоны /
33 050,616 кг; минимум зон — 20 / 108 070,886 кг. Начальный просмотр по
исследовательской ridge-модели выбрал индекс 99: 239 зон, 35 260,861 кг и
3258 физических стержней; индекс исходной графики совпал. Времена поиска по
четырём направлениям составили 15,60 / 15,41 / 11,16 / 15,19 с. Конечная
пересборка во всех направлениях вернулась к базовому фронту из-за лимита пула
15 000 предложений (`pool_limit`): улучшение MILP здесь не заявляется. Раскрой
партии не прошёл у всех 141 вариантов, `placement_eligible=false`. Массу
прямых дополнительных стержней алгоритма нельзя сравнивать с полным PDF,
включающим гнутые и наклонные позиции; новой метки обучения этот расчёт не даёт.
