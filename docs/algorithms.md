...
`algorithms/composite_recombine.py` отдельно пересобирает зоны из ограниченного
пула проверенных предложений конечным cover-MILP; он сохраняет огибающую
базового иерархического фронта по каждому прежнему бюджету зон и добавляет не
более двух точек после полного checker. Доминируемые базовые точки удаляются.
Общеплитный DP использует до 18 точек каждого направления и показывает
