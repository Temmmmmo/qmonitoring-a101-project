...
Статус: baseline и первый genetic search на 2026-08-24. Документ объясняет, какие алгоритмы уже есть,
...
5. комбинирует разные допустимые решения для `bottom/top × X/Y`;
...
1. регулярная маска и начальные атомы строятся теми же сервисами, что у
   `spatial-partition-greedy`;
2. один merge-проход сохраняет выбранные и несколько альтернативных допустимых bbox;
3. хромосома — множество индексов прямоугольников CandidateSet;
4. crossover смешивает множества родителей, mutation добавляет, удаляет или заменяет
   прямоугольник;
5. детерминированный repair закрывает все исходные атомы, удаляет избыточные гены и
   пытается уложиться в `max_details`;
6. NSGA-II-подобный ranking и crowding одновременно минимизируют массу и число зон;
7. финальные особи материализуются через общий builder, проходят фазирование и затем
   полностью перепроверяются `evaluate_layout`;
8. внешний `build_direction_pareto_front` повторно удаляет невалидные, доминируемые и
...
`candidate_window` (6). Одинаковые вход, параметры и seed дают одинаковый фронт.

На реальном `С1_t_800_Верхняя по оси У` при `max_details=32`, population 16,
30 поколениях и seed 42 получено 9 прошедших hard-валидацию точек: от 23 зон / 7123,0 кг
до 31 зоны / 6385,6 кг; недоармированных КЭ нет. Это приближённый фронт внутри текущего
CandidateSet, а не доказательство глобального оптимума. Альтернативные топологии пока
ограничены прямоугольниками одного пространственного прохода; следующий контроль —
exact/branch-and-bound oracle на малых масках и сравнение optimality gap.

Научный консультант также упомянул обучаемый оператор мутации. Он намеренно не входит в
первую версию: обычные операторы фиксируют понятный и воспроизводимый baseline.
