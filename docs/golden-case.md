...
На плане инженер показывает не цветные прямоугольники, а группы параллельных стержней.
Подпись группы содержит позицию, диаметр, шаг и количество, например
`П5 ⌀12, шаг 150 (7 шт.)`. Спецификация справа содержит:
...
стержней в плане в пределах зоны усиления и запрещена их обрезка. Это подтверждает, что
область потребности, параметрическая зона и фактические линии стержней — связанные, но
не тождественные геометрии.
...
- Эталонные DXF не содержат `.shk`, поэтому текущий оптимизатор нельзя честно запустить
  на этой плите без согласованного соответствия диапазонов `As → (диаметр, шаг)`.
