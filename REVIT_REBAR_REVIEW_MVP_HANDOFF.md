...
[`qmonitoring-rebar-review-mvp-0.1.3-code-only.zip`](downloads/qmonitoring-rebar-review-mvp-0.1.3-code-only.zip).
...
4. Выбрать полный graphic JSON, новый путь JSON-отчёта и явный XY.
   Выбрать точные загруженные RebarBarType для каждого steel/D, подтвердить
   автоматически предложенные глубины; ручной ввод только «Другие настройки».
...
Для текущего К09 автоматическое диагностическое предложение: **`46;62;48;68` мм**
(низ X; низ Y; верх X; верх Y), от native-грани до ОСИ, не cover; 0 блокируется.
Оси Z: 6956;6972;7062;7042 мм. Dmax: низ 12 / верх 16 мм. Минимальный вертикальный зазор
между ортогональными телами: низ 4 / верх 4 мм, верх-низ 56 мм. Все 918 проходят именно
...
0.1.3: ручной ввод не обязателен. Профиль MVP задаёт отступ тела40мм и зазор4мм,
не Revit cover/норматив; глубины вычисляются по Dmax выбранных native-типов.
Подтверди предложение или выбери «Другие настройки». Auto-профиль точно
пересчитывается перед созданием по актуальным native-граням/типам, manual не
получает auto-gap approval. Report сохраняет policy/computed/actual/auto-manual,
в том числе при blocked_preflight. Необходимая толщина для всехØ36 —236мм.
