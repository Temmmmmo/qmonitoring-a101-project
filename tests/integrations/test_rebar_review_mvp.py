...
def host_data(size=10000):
    def loop(z):
        ...
        return [{"kind": "Line", "start_mm": a, "end_mm": points[(i + 1) % 4], "length_mm": size}
            for i, a in enumerate(points)]
    return {"element_id": 99, "covers": {key: {"distance_mm": 40} for key in ("top", "bottom", "other")},
        "top_faces": [{"plane": {"origin_mm": [0, 0, 300], "normal": [0, 0, 1]}, "edge_loops": [loop(300)]}],
        "bottom_faces": [{"plane": {"origin_mm": [0, 0, 0], "normal": [0, 0, -1]}, "edge_loops": [loop(0)]}]}


def primitives():
    bars = []
    for index, direction in enumerate(("bottom-X", "bottom-Y", "top-X", "top-Y")):
        start, end = ([1000, 1000 + index * 500], [3000, 1000 + index * 500]) if direction.endswith("X") else ([1000 + index * 500, 1000], [1000 + index * 500, 3000])
        bars.append({"direction": direction, "bar_id": "bar-" + direction, "start_xy_mm": start,
            "end_xy_mm": end, "diameter_mm": 12, "steel_class": "A500", "source_refs": [direction]})
    mass = sum(.000006165 * 12 ** 2 * 2000 for _ in bars)
    return {"input_schema": "graphic-bar-plan-draft/v1", "placement_eligible": False,
        "engineering_approval": False, "bars": bars, "summary": {"physical_bar_count": 4,
            "additional_mass_kg": mass, "position_count": 1, "source_zone_count": 4}}
...
def native(plan):
    ...
    for index, run in enumerate(plan["runs"]):
        line = run["axes"][0]
        rows.append({"element_id": 1000 + index, "host_id": 99, "quantity": 1, "number_of_bar_positions": 1,
            "layout_rule": "Single", "hook_type_ids": [-1, -1], "review_bar_id": run["bar_id"],
            "review_direction": run["direction"], "bar_type": {"element_id": 7, "nominal_diameter_mm": 12,
            "model_diameter_mm": 12}, "bars": [{"position_index": 0, "curves": [{"kind": "Line",
            "start_mm": line["start_mm"][:], "end_mm": line["end_mm"][:], "length_mm": 2000}]}]})
    return rows
...
def make_plan(contract):
    ...
    return contract.make_review_plan(primitives(), host, types, dict.fromkeys(contract.DIRECTIONS, 50))
...
def test_flat_native_outer_plan_and_complete_native_readback(contract):
    ...
    assert result["status"] == "matches" and result["tolerance_mm"] == .01
...
def test_native_edges_can_be_reversed_and_reordered_without_changing_contour(contract):
    ...
    assert {tuple(p) for p in result["outer_xy_mm"]} == {(0, 0), (10000, 0), (10000, 10000), (0, 10000)}


def test_vertex_split_narrow_concave_bay_cannot_enter_bar_body(contract):
    polygon = [[-1, -1], [11, -1], [11, 10], [3, 10], [3, 6], [3, 4],
        [2, 4], [2, 6], [2, 10], [-1, 10]]
    ...
    assert contract._corridor_contained([0, 4], [10, 4], 2,
        [[0, 2], [10, 2], [10, 6], [0, 6]]) is True
...
def test_native_readback_tamper_is_rejected(contract, change):
    ...
    if change == "missing":
        ...
        rows[0]["bars"][0]["curves"][0]["length_mm"] += .02
    else:
...
@pytest.mark.parametrize("change", ("slope", "step", "curve", "disconnected", "outside", "depth", "bar-type"))
def test_unsupported_host_or_party_blocks_whole_plan(contract, change):
    ...
    if change == "slope":
        data["top_faces"][0]["plane"]["normal"] = [.1, 0, 1]
    elif change == "step":
...
def test_real_s1_graphics_is_complete_and_keeps_visible_failures(contract):
    ...
    assert all(result["trim_graphics"]["checks"][key] == "fail"
...
def test_python2_grammar_and_no_save_sync_delete_or_blanket_obstacle_scan():
    ...
    for relative in ("qm_rebar_review.py", "qm_revit_rebar_review.py"):
        ...
        for forbidden in (".Save(", ".SaveAs(", "SynchronizeWithCentral(", ".Delete(", "find_obstacles("):
...
def runtime_harness(monkeypatch, contract):
    runtime = importlib.import_module("qm_revit_rebar_review")
    state = SimpleNamespace(ids={7, 99}, rebars={}, calls=[], fail_at=None, tamper_after=False,
        read_count=0, commit="Committed", issue_section=None)
    document = SimpleNamespace(IsFamilyDocument=False, IsReadOnly=False, IsModifiable=False,
        IsWorkshared=False, Title="COPY", Application=SimpleNamespace(VersionNumber="2024"))
    ...
    document.GetElement = lambda value: floor if value.Value == 99 else state.rebars.get(value.Value)
    ...
    db = SimpleNamespace(Floor=Floor, ElementId=Id,
        Structure=SimpleNamespace(RebarBarType=BarType, Rebar=Rebar),
        Transaction=Scope, TransactionGroup=Scope,
    ...
    class Probe:
        ...
        def floor(self, _):
            if state.issue_section:
                self.issues.append({"section": state.issue_section, "message": "injected incomplete read"})
    ...
    def create(probe, host, typ, run, factory):
        ...
        item.Id = Id(value)
        item._comment = None
        item.get_Parameter = lambda _: SimpleNamespace(IsReadOnly=False, Set=lambda text: setattr(item, "_comment", text) is None)
    ...
    def read_created(probe, ids, identities):
        ...
        if state.tamper_after and state.read_count == 2:
            rows[0]["bars"][0]["curves"][0]["end_mm"][0] += .02
        return rows
    ...
    monkeypatch.setattr(runtime, "inspect_trial_worksharing", lambda *_: {"status": "not_applicable", "mode": "non_workshared"})
...
@pytest.mark.parametrize("mode", ("keep", "decline", "mid-batch", "post-tamper", "pending", "cover", "geometry-read"))
def test_transaction_keep_or_full_rollback(runtime_harness, contract, mode):
    ...
    state.issue_section = "floor/cover/top" if mode == "cover" else "floor/top_faces" if mode == "geometry-read" else None
    source = primitives()
    source.update({"input_schema": "graphic-bar-plan-draft/v1", "case_id": "case",
        "trim_graphics": {"checks": {"coverage": "fail"}}, "source_blockers": ["not-approved"]})
    result = runtime.run_rebar_review(doc, db, floor, source, {"A500|12": typ},
        dict.fromkeys(contract.DIRECTIONS, 50), lambda: [], mode in ("keep", "cover"), copy_confirmed=True)
...
def test_code_only_package_is_deterministic_and_dependency_complete(tmp_path, monkeypatch):
    ...
    with ZipFile(first) as archive:
        ...
        assert manifest["version"] == "0.1.2" and manifest["creates_structural_rebar"] is True
...
def test_exact_element_id_and_comment_binding_are_read_back(monkeypatch):
    ...
    probe = SimpleNamespace(DB=SimpleNamespace(ElementId=lambda value: value,
        Structure=SimpleNamespace(Rebar=Rebar), BuiltInParameter=SimpleNamespace(
            ALL_MODEL_INSTANCE_COMMENTS="comments")), doc=SimpleNamespace(GetElement=lambda _: rebar))
