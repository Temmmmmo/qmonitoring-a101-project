...
def render_composite_svg(demand, zones, *, host_envelope=None, outline_cell_ids=()) -> str:
    boxes = [demand.bbox]
    ...
    for zone in zones:
        for component in zone.components:
            ...
            for coord in pattern_coordinates(component.placement, component.axis_window_mm):
                ...
                boxes.append(box)
                lines.append((box, zone, component))
    ...
    bars = []
    for (x1, y1, x2, y2), zone, component in lines:
        color = ZONE_COLORS[component.component_index % len(ZONE_COLORS)]
        bars.append(f'<line x1="{x1-xmin:.3f}" x2="{x2-xmin:.3f}" y1="{ymax-y1:.3f}" y2="{ymax-y2:.3f}" '
                    ...
                    f'{html.escape(zone.id)}; добавка {component.component_index + 1}; Ø{component.rebar.diameter}; '
                    f'L={component.installed_length_mm:.3f} мм; условный шаг {component.rebar.step}</title></line>')
    contours = []
    if host_envelope is not None:
    ...
    label = "Фактические оси" if zones else "Исходный спрос; раскладка отсутствует"
