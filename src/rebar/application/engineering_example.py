...
def example_metadata(*, available: bool, status: str) -> dict:
    return {
        ...
        "profile": {
            ...
            "note": "Фон — основная сетка Ø10@300 из шкалы проекта. Рассчитываются только добавки. "
                    "Для примера приняты начало фона 0 мм, сдвиг первой добавки @300 — 100 мм, второй — 0 мм. "
                    "Первая добавка @150 размещается через интервалы 100/200 мм между фоновыми осями. "
                    "Вводить эти параметры для готового примера не нужно. Контур берётся из DXF; "
                    "для проверки границ настоящей плиты нужен Working Host из Revit. "
                    "Отверстия и защитный слой вне MVP. Высоты, фон в модели и совместная укладка требуют "
                    "проверки конструктора. Это выбранный профиль примера, не универсальное правило проекта.",
        },
    }
