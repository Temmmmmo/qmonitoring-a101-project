...
from ..services.tz_boundary_trim import POLICY, build_trimmed_pieces, straight_outer_intersections, trimming_parent


def trim_straight_bars_to_outer_boundary(before, actual_host, *, lanes=None, nudge_edge_axis=False,
                                        discard_empty_intersections=False):
    ...
    for bar in before:
        ...
        mapping.append({"direction": str(bar.direction), "source_bar_id": bar.id,
                        "piece_ids": [b.id for b in pieces], "policy": POLICY})
        if len(after) > 10000:
