...
Наша задача ближе всего не к упаковке прямоугольников (`rectangle packing`), а к
**взвешенному прямоугольному покрытию с конфликтами**:
...
- две выбранные зоны одного направления пока не должны пересекаться;
...
или covering integer program. Запрет пересечений добавляет конфликтные ограничения
`x_r + x_s <= 1`, а инженерные правила должны отбрасывать недопустимые кандидаты ещё до
запуска оптимизатора.

## Что реализовывать следующим

### 0. Общий builder/validator и `CandidateSet`

Сначала завершить обязательный слой из `docs/optimization-notes.md`: ширина `k × step`,
`bar_count = k + 1`, длина с `40d`, покрытие КЭ, непересечение, зазоры и масса. Затем
исчерпывающе перечислять прямоугольники по допустимым границам КЭ и конфигурациям
`(diameter, step)`. Это даст общий вход всем следующим методам.
...
конфликтные ограничения для пересечений, лимит зон и минимизация массы. MILP удобен
...
### 6. `multilevel-benders` — только при аддитивной семантике пересечений

Если конструктор подтвердит, что `As` нескольких пересекающихся деталей суммируется, карту
...
пересечения запрещены и одна зона должна самостоятельно удовлетворять локальный уровень,
такой модуль реализовывать не следует.
...
- ML до появления детерминированного валидатора и набора инженерных эталонов не должен
  генерировать зоны. Позже он может выбирать веса Парето-критериев или порядок локального
  поиска.
